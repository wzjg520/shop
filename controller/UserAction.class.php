...
class UserAction extends Action{
	...
	public function reg(){
		if(isset($_POST['send'])){
			if($this->model->add()){
				$this->redirect->success('会员注册成功','?a=user&m=login');
			}else{
	...
	public function loginOut(){
		$this->model->loginOut() ? $this->redirect->success('','./') : $this->redirect->error('退出失败！');
	}
	//显示收藏商品
	...
	public function addCollect(){
		if(isset($_COOKIE['user'])){
			$this->collect->runAdd() ? $this->redirect->success('','?a=user&m=mycollect') : $this->redirect->error('收藏失败');
		}else{
			$this->redirect->error('请登录后再操作<a href="?a=user&m=login">[登陆]</a>');
		}	
	}
	//取消收藏
	public function deleteCollect(){
		if(isset($_GET['id']))$this->collect->delete() ? $this->redirect->success('','?a=user&m=mycollect') : $this->redirect->error('操作失败');
	}
	//显示并添加收货人地址
	public function address(){		
		if(isset($_POST['send']) && !isset($_GET['id']))$this->address->add() ? $this->redirect->success('新增成功','?a=user&m=address') : $this->redirect->error('添加失败');
		if(isset($_POST['send']) && isset($_GET['id'])) $this->address->update() ? $this->redirect->success('修改成功','?a=user&m=address') : $this->redirect->error('修改失败');
		$this->tpl->assign('frontTenNav',$this->nav->getFrontTenNav());
	...
	public function selected(){
		if(isset($_GET['id']))$this->address->selected() ? $this->redirect->success('','?a=user&m=address') : $this->redirect->error('操作失败');
	}
	//删除收货人
	public function runDelete(){
		if(isset($_GET['id']))$this->address->delete() ? $this->redirect->success('','?a=user&m=address') : $this->redirect->error('操作失败');
	}
	//显示订单列表
	...
	public function commend(){
		if(isset($_POST['send']))$this->commend->add() ? $this->redirect->success('',PREV_URL) : $this->redirect->error('发表评论失败');
	...
	public function cancel(){
		if(isset($_GET['id'])){
			$this->order->cancel() ? $this->redirect->success('','?a=user&m=order') : $this->redirect->error('执行出错！');
		} 
	}
	//申请退款
	public function refund(){
		if(isset($_GET['id'])){
			$this->order->refund() ? $this->redirect->success('',PREV_URL) : $this->redirect->error('执行出错！');
		}
	}
	//删除订单
	public function deleteOrder(){
		if(isset($_GET['id'])){
			$this->order->delete() ? $this->redirect->success('','?a=user&m=order') : $this->redirect->error('请确认订单是否已取消！');
		}
	}
	//ajax验证验证码是否正确

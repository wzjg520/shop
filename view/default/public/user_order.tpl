	...
	<table>
			...
			{if  $value->order_state=='已取消'}
					...
					{else}
						{if $value->order_delivery=='已发货'}
							{if $value->refund==1}
								<span style="color:red">退款处理中</span>
							{elseif $value->refund==2}
								<span style="color:green">退款成功</span>
							{else}
								<span style="color:green">已发货，等待收货</span>
							{/if}							
						{else}
							{if $value->order_delivery=='已配货'}
								...
								{elseif $value->refund==2}
									<span style="color:green">退款成功</span>
								{else}
									<span style="color:green">已配货，等待发货</span>
								{/if}							
							{else}
								{if $value->order_pay=='已付款'}	
									...
									{elseif $value->refund==2}
										<span style="color:green">退款成功</span>
									{else}
										<span style="color:green">已付款，等待配货</span>
									{/if}																	
														
								{else}
									{if $value->order_state=='已确认'}
										<span style="color:green">已确认，等待付款</span>
									{else}
		...
		<td>
			{if $value->order_delivery=='已配货' || $value->order_state=='已取消' || $value->order_pay=='已付款'}	
				...
				<a style="color:red;" href="?a=user&m=deleteOrder&id={$value->id}" onclick="return confirm('您确定要删除吗') ? true : false">删除</a>
				{elseif $value->refund=='0'}
				<a style="color:red;" href="?a=user&m=refund&id={$value->id}" onclick="return confirm('申请退款须知：\n\n1.已付款、已配货或已发货可申请退款；\n2.已付款或已配货的 72 小时确认退款；\n3.已发货的请寄回物品后 72 小时确认退款；\n\n您真的要申请退款吗？') ? true : false">申请退款</a> 
				{else}
				----
				{/if}
			{else}
